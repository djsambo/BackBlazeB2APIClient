...
app.uploadFile = (xbzFileName, contentType, contentLength, xbzContentSha1, xbzInfoSrcLastModifiedMillis, xbzInfoB2ContentDisposition, xbzInfoX) => {
    return new Promise((resolve, reject) => {
        if (!_configs.hasOwnProperty("authData")) {
            throw Error("You should authorize the account first.");
        }
...
app.uploadPart = (xbzPartNumber, contentLength, xbzContentSha1) => {
    return new Promise((resolve, reject) => {
        if (!_configs.hasOwnProperty("authData")) {
            throw Error("You should authorize the account first.");
        }
